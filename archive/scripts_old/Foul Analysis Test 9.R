...
# ==================== LOAD EVENT ====================
file_path <- here("Raw PBP Data", "UFC-imavov-borralho-06-09-2025.html")
...
groin_patterns <- c(
  ...
  # 🔹 NEW: victim phrasing – “takes one to the groin”
...
eye_patterns <- c(
  ...
  # 🔹 NEW: generic motion into the eye / eye socket
  "(slides?|scrapes?|rakes?|grazes?|jabs?|jabbed|jabbing|stabs?).{0,30}(?:into|in|to).{0,10}(?:the\\s+)?(eye|eye socket)"
)
...
write_csv(fouls_df9, "foul_detection_output_event9.csv")

fouls_df9

cat("\n✅ Event 9 analysis complete!\n")
cat(sprintf("Total fouls detected: %d\n", nrow(fouls_df9)))
cat(sprintf("Unknown foulers: %d\n", sum(fouls_df9$foul_committed_by == "UNKNOWN")))
