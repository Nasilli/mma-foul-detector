...
# ==================== LOAD EVENT ====================
file_path <- here("Raw PBP Data", "UFC321-aspinall-gane-25-10-2025.html")
...
eye_patterns <- c(
  ...
  # generic “X jams into the eyes”
  "(slides?|scrapes?|rakes?|grazes?|jabs?|jabbed|jabbing|stabs?|stabbing|jams?|jammed|jamming).{0,30}(?:into|in|to).{0,10}(?:the\\s+)?(eye|eyes|eye socket|eye sockets)"
)
# general "back of head" anchor (still used later)
back_of_head_pat <- "back of (?:the |his |her )?head"

# explicit attack → back of the head
back_head_patterns <- c(
  "(?:elbow|strike|punch|kick|knee|hammerfist|shot|blow|club).{0,80}back of (?:the |his |her )?head",
  "back of (?:the |his |her )?head.{0,40}(?:illegal|foul|shot|strike|elbow|punch|kick|knee)"
)
...
sent_all <- pbp_tbl %>%
  ...
  mutate(
    ...
    # “reaction-only” lines (hurt / clutching, not a new foul)
    is_reaction_only = str_detect(sentence, reaction_only_re),
    # final foul flag used everywhere else
    has_foul         = has_foul_raw & !is_reaction_only,
...
strong_foul_anchor_re <- regex(
  paste(
    c(
      ...
      "\\beye(s)?\\b",
      "headbutt",
      back_head_patterns,                 # <- now just another element in the vector
      "rabbit punch",
      # toes + structure
      "toes? in the cage",
      "toes? in the fence",
...
detect_fouler_v5 <- function(context, fighter1, fighter2, bout = NA) {
  ...
  victim_eye_jab <- function(name) {
    paste0(
      "(?:jabs?|jabbed|jabbing|pokes?|poked|stabs?|thumbs?)\\s+",
      "\\b", name, "\\b.{0,25}(?:in|into)\\s+the\\s+eye"
    )
  }
...
write_csv(fouls_df10, "foul_detection_output_event10.csv")

fouls_df10

cat("\n✅ Event 10 analysis complete!\n")
cat(sprintf("Total fouls detected: %d\n", nrow(fouls_df10)))
cat(sprintf("Unknown foulers: %d\n", sum(fouls_df10$foul_committed_by == "UNKNOWN")))

context10 <- fouls_df10 %>% 
  select(context)