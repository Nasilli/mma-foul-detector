...
# ==================== LOAD EVENT ====================
file_path <- here("Raw PBP Data", "UFCvegas-106-burns-morales-17-05-2025.html")
...
# FIX: Removed "clash of heads" - not a foul, added toes in cage
foul_patterns <- c(
  groin_patterns,
  eye_patterns,
  "head ?butt",
...
eye_categorise_re <- regex(paste("\\beye poke\\b|\\beyepoke\\b", "finger(s)?.{0,30}(?:to|into|in|slides?|scrapes?|rakes?|grazes?|touches?).{0,10}(?:the )?eye", "\\bpoke.{0,15}eye\\b", "\\bwatch\\s+his\\s+fingers\\b", sep = "|"), ignore_case = TRUE)
...
fouls_core <- sent_runs_clean %>%
  ...
  # must both match a foul pattern *and* contain a strong foul anchor
  filter(
...
detect_fouler_v5 <- function(context, fighter1, fighter2, bout = NA) {
  ...
  txt <- str_replace_all(context, "[\u2018\u2019\u201c\u201d`']", "'") %>% stringr::str_to_lower()
  has <- function(p) stringr::str_detect(txt, stringr::regex(p, ignore_case = TRUE))
  # ---------- RULE 0.1: direct "NAME ... kick ... cup" low blow ----------
  # ---------- RULE 0: explicit "NAME ... [spinning] kick ... into the cup" ----------
  # ---------- RULE 0.1: direct "NAME ... kick ... cup" low blow ----------
  ...
  hit_low_1 <- has(direct_low_blow(f1_last))
  hit_low_2 <- has(direct_low_blow(f2_last))
  ...
  pat_complain <- function(name) paste0("\\b", name, "\\b.{0,40}", complain_verbs, ".{0,40}", foul_words)
  ...
  # NEW: Handle "NAME ... verb ... that [pounds/lands] into OBJECT" patterns
  ...
  score1 <- 3*has(pat_actor(f1_last)) + 3*has(pat_actor_that(f1_last)) + 2*has(pat_actor_loose(f1_last)) + 3*has(pat_fence(f1_last)) + 3*has(pat_fence_poss(f1_last)) + 3*has(pat_actor_poss(f1_last)) + 3*has(pat_by(f1_last)) - 3*has(pat_victim(f1_last))
  score2 <- 3*has(pat_actor(f2_last)) + 3*has(pat_actor_that(f2_last)) + 2*has(pat_actor_loose(f2_last)) + 3*has(pat_fence(f2_last)) + 3*has(pat_fence_poss(f2_last)) + 3*has(pat_actor_poss(f2_last)) + 3*has(pat_by(f2_last)) - 3*has(pat_victim(f2_last))
  ...
  if (!is.na(foul_match[1])) {
    ...
    if (!is.na(last_fighter)) {
      # look at the region from that last mention up to the foul anchor
      region_after_last <- stringr::str_sub(txt_before_foul, last_fighter_pos, nchar(txt_before_foul))
      has_action_verbs <- stringr::str_detect(region_after_last, stringr::regex(verbs_attack, ignore_case = TRUE))
      has_continuation  <- stringr::str_detect(region_after_last, stringr::regex("\\band\\s+(starts?|begins?|continues?|proceeds?|keeps?|bludgeon)", ignore_case = TRUE))
      has_pronoun       <- stringr::str_detect(region_after_last, stringr::regex("\\b(him|her|them)\\b\\s+with\\s+(punches?|kicks?|strikes?|knees?|elbows?)", ignore_case = TRUE))
  ...
  # FALLBACK
  foul_anchor_re <- regex("cup|groin|eye|fence|cage|back of the head|grounded opponent|illegal knee|eye poke", ignore_case = TRUE)
...
write_csv(fouls_df3, "foul_detection_output_event3.csv")
fouls_df3

cat("\n✅ Event 3 analysis complete!\n")
cat(sprintf("Total fouls detected: %d\n", nrow(fouls_df3)))
cat(sprintf("Unknown foulers: %d\n", sum(fouls_df3$foul_committed_by == "UNKNOWN")))
