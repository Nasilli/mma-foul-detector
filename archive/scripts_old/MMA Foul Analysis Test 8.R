### DUE TO A MISTAKE IN THE PBP COMMENTARY, THE EMMETT EYE POKE WAS ACTUALLY DONE BY MURPHY - CHANGE THIS WHEN TABLES ARE JOINED BEFORE ANALYSIS
# ---- packages ----
library(rvest)
library(readr)
library(here)
...
# ==================== LOAD EVENT ====================
file_path <- here("Raw PBP Data", "UFCvegas-105-emmett-murphy-05-04-2025.html")
...
pause_patterns <- c(
  ...
  # refs explicitly "call time"
  "\\b(?:tognoni|herzog|goddard|mazzagatti|hatley|peterson|beltran|rosenthal|dean|herb\\s+dean)\\b.{0,20}calls? time\\b",
  # 'time he needs to recover'
  "\\btime\\s+\\w+\\s+needs?\\s+to\\s+recover\\b",
  # 'takes 30 seconds to clear his vision / recover'
...
sent_runs <- sent_all %>%
  filter(has_foul) %>%             # keep only foul sentences
  group_by(bout) %>%
  mutate(
    run_id           = row_number(),          # 1,2,3,... per bout
...
strong_foul_anchor_re <- regex(
  paste(
    ...
    "illegal upkick",
    "illegal (?:kick|upkick) to (?:a )?grounded opponent",
    # 🔹 NEW: crown-of-head / generic illegal strike
    "crown of the head",
    "illegal strike",
    # 🔹 NEW: toes-foul phrasing from Alencar vs Demopoulos
...
write_csv(fouls_df8, "foul_detection_output_event8.csv")

fouls_df8

cat("\n✅ Event 8 analysis complete!\n")
cat(sprintf("Total fouls detected: %d\n", nrow(fouls_df8)))
cat(sprintf("Unknown foulers: %d\n", sum(fouls_df8$foul_committed_by == "UNKNOWN")))
