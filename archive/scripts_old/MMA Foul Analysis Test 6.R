...
# ==================== LOAD EVENT ====================
file_path <- here("Raw PBP Data", "UFC-lewis-teixeira-12-06-2025.html")
...
pause_patterns <- c(
  ...
  # NEW: "gives Curtis time to recover" style
  "\\bgives?\\s+\\w+\\s+time to recover\\b",
...
detect_fouler_v5 <- function(context, fighter1, fighter2, bout = NA) {
  ...
  victim_passive_any <- function(name) {
    paste0(
      "\\b", name, "\\b.{0,80}",
      "(was|is|gets?|got|were)\\s+",
      "(hit|struck|headbutted|kicked|kneed|poked)\\b"
    )
  }
...
write_csv(fouls_df6, "foul_detection_output_event6.csv")

fouls_df6

cat("\n✅ Event 6 analysis complete!\n")
cat(sprintf("Total fouls detected: %d\n", nrow(fouls_df6)))
cat(sprintf("Unknown foulers: %d\n", sum(fouls_df6$foul_committed_by == "UNKNOWN")))
